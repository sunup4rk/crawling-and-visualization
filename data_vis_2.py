...
cutoff1 = (((df['RBI'] > df['PA'] * 0.15)) & (df['RBI'] > 25)) | (((df['RBI'] > df['PA'] * 0.11)) & (df['PA'] > 200)) | (df['RBI'] >= 30)
...
for i, txt in enumerate(players_NAME):
    if ((players_RBI[i] > players_PA[i] * 0.15) and (players_PA[i] > 150 or players_RBI[i] > 25))\
    or ((players_RBI[i] > players_PA[i] * 0.11) and (players_PA[i] > 200)) or players_RBI[i] >= 30:
        plt.annotate(txt, (players_PA[i], players_RBI[i]))
    else:
        pass
